...
#include <stdlib.h>
#include <string.h>
#pragma warning(disable:4996) //컴파일 오류 무시
...
int main(void)
{
	...
	while (pc < 64)
	{
		...
		mux_result = Mux(control.ALUSrc, reg_read.Read_data_2, inst_ext_32);
		...
		mux_result = Mux(control.Branch & alu.zero, pc_add_4, pc_add_inst);
		...
		// register write
		Register_Write(control.RegWrite, Mux(control.RegDst, inst_20_16, inst_15_11), mux_result);
...
unsigned char ALU_Control_Signal(unsigned char signal)
{
	if (signal == 2 || signal == 0) //ADD, LW, SW
		return '+';
	else if (signal == 1) // BEQ
		return '-';
	else return 0;
}
...
unsigned int Memory_Access(unsigned char MemWrite, unsigned char MemRead, unsigned int addr, unsigned int write_data)
{
	...
	if (MemRead == 1) //addr에서 data 읽기 (LW)
		Read_data = mem[addr];
	if (MemWrite == 1)//addr에 data 저장(SW) => memory에만 쓰고 끝
		mem[addr] = write_data;
